class Boid {
		...
		boids.forEach((boid) => {
			if (boid != this) {
				...
				if (dir.mag() <= this.perceptDist && dot > this.visionRange) {
					if (boid.col == this.col) this.sameBoids.push(boid);
					else this.diffBoids.push(boid);
				}
			}
		});
	}
	...
	update() {
		...
		if (this.position.x < (-width - this.size) / 2.0) this.position.x = (width + this.size) / 2.0;
		else if (this.position.x > (width + this.size) / 2.0) this.position.x = (-width - this.size) / 2.0;

		// Check top and bottom edges
		if (this.position.y < (-height - this.size) / 2.0) this.position.y = (height + this.size) / 2.0;
		else if (this.position.y > (height + this.size) / 2.0) this.position.y = (-height - this.size) / 2.0;
	}
