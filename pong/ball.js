class Ball {
    ...
    checkhitTab() {
        if (this.velocity.y < 0 && this.position.y <= tabThick + tabYPos) {
            if (this.position.x < tabA.position.x + tabWide / 2 &&
                this.position.x > tabA.position.x - tabWide / 2)
                return abs(this.position.x - tabA.position.x) / tabWide;
        }

        else if (this.velocity.y > 0 && this.position.y >= height - tabThick - tabYPos) {
            if (this.position.x < tabB.position.x + tabWide / 2 &&
                this.position.x > tabB.position.x - tabWide / 2) {
